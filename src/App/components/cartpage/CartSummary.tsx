...
function countDiscountPercent(discounts: string[]) {
  let totalPercent = 0;
...
function CartSummary({ setIsPurchasePopUpEnabled, totalItems, totalPrice }: TProps) {
  ...
  useEffect(() => {
    setTotalDiscount(countDiscountPercent(discounts));
  }, [discounts]);
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <button type="button" className={styles.button__buy} onClick={() => setIsPurchasePopUpEnabled(true)}>
        To Checkout...
      </button>
    </div>
  );
